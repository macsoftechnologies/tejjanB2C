    ...
    if (vehicleTypes != undefined && vehicleTypes.length > 0) {
      ...
      vehicleTypes.forEach(vehicle => {
        ...
        vehicle.categories.forEach(cate => {
          ...
          if (cate.additionalServices != undefined && cate.additionalServices != null) {

            cate.additionalServices.forEach(service => {

              service.displayRateInfo.forEach(rate => {

                if (rate.purpose == 10)

                  addServiceAmount += rate.amount
      ...
      let vehicleObj = {
        ...
        totalAmount: totalAmount + addServiceAmount
  ...
  gorundService(categorie){
    ...
    var VAT = 0
     ...
     if(categorie != undefined && categorie != null){
           ...
           categorie.displayRateInfo.forEach(rate =>{

            if(rate.purpose == 1)
    
            groundAmount += rate.amount
